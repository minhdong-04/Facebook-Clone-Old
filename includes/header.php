...
if (!$current_user) {
    // Nếu không lấy được user → đăng xuất an toàn
    session_destroy();
    header("Location: ../auth/login.php");
    exit;
}
...
$currentUserNameSafe = fb_escape((string)($current_user['name'] ?? ''));
$currentUserAvatar = fb_avatar_url($current_user['avatar'] ?? null);

$activeScript = basename($_SERVER['PHP_SELF'] ?? '');
$activeKey = match ($activeScript) {
    'home.php' => 'home',
    'friends.php' => 'friends',
    'watch.php' => 'watch',
...
$pageTitleSafe = fb_escape((string)($pageTitle ?? 'Facebook'));
?>
...
<head>
        ...
        (function () {
            try {
                ...
                if (mq) {
                    const onChange = () => {
                        try {
                            const v = localStorage.getItem(KEY) || 'off';
                            if (v === 'auto') apply();
                        } catch (_e) {}
                    };
                    try {
    ...
    <style>
        ...
        .account-popover .acct-menu .see-all-btn:hover { background: rgba(0, 0, 0, .05); border-radius: 8px; }
...
<header class="header">
    ...
    <div class="center-nav-wrap" aria-hidden="false">
        <nav class="center-nav" role="navigation" aria-label="Main">
            <a class="cnav-item <?= $activeKey === 'home' ? 'active' : '' ?>" href="../pages/home.php" title="Trang chủ" data-key="home" aria-current="<?= $activeKey === 'home' ? 'page' : 'false' ?>">
            ...
            <a class="cnav-item <?= $activeKey === 'friends' ? 'active' : '' ?>" href="../pages/friends.php" title="Bạn bè" data-key="friends" aria-current="<?= $activeKey === 'friends' ? 'page' : 'false' ?>">
            ...
            <a class="cnav-item <?= $activeKey === 'watch' ? 'active' : '' ?>" href="../pages/watch.php" title="Watch" data-key="watch" aria-current="<?= $activeKey === 'watch' ? 'page' : 'false' ?>">
            ...
            <a class="cnav-item <?= $activeKey === 'market' ? 'active' : '' ?>" href="../pages/marketplace.php" title="Marketplace" data-key="market" aria-current="<?= $activeKey === 'market' ? 'page' : 'false' ?>">
            ...
            <a class="cnav-item <?= $activeKey === 'groups' ? 'active' : '' ?>" href="../pages/group.php" title="Nhóm" data-key="groups" aria-current="<?= $activeKey === 'groups' ? 'page' : 'false' ?>">
...
<?php include __DIR__ . '/messenger_popover.php'; ?>

<script>
    // Expose current user for shared JS (Messenger popover / realtime chat)
    window.CURRENT_USER_ID = <?= (int)($current_user['id'] ?? 0) ?>;
</script>

<script src="https://cdn.socket.io/4.7.2/socket.io.min.js"></script>

<script>
    // ===== Realtime presence (shared, site-wide) =====
    (function () {
        try {
            const uid = Number(window.CURRENT_USER_ID || 0);
            if (!uid || typeof window.io !== 'function') return;

            // Keep DB last_active fresh so get_contacts.php can show online state.
            ...
            // Reuse existing socket if a page already created one.
            if (!window.fbSocket) {
                ...
                // If HTTPS (reverse-proxy), use same-origin. Otherwise connect to :3000.
                const useSameOrigin = (proto === 'https:');
                window.fbSocket = useSameOrigin ? io(undefined, socketOpts) : io(`http://${host}:3000`, socketOpts);
            }

            const socket = window.fbSocket;
            if (!socket || typeof socket.emit !== 'function') return;

            // Helper: apply presence to contacts list (if present on the page).
            window.fbPresenceApply = function (userId, online) {
                const id = String(userId || '').trim();
                if (!id) return;

                const safeId = (window.CSS && typeof window.CSS.escape === 'function') ? window.CSS.escape(id) : id;
                const rows = document.querySelectorAll(`.fb-contact-item[data-user-id="${safeId}"]`);
                rows.forEach((row) => {
                    if (!row || !row.classList) return;
                    row.classList.toggle('is-online', !!online);
                    try { row.dataset.userOnline = online ? '1' : '0'; } catch (_e) {}

                    const last = row.querySelector('.fb-contact-last');
                    if (last) last.style.display = online ? 'none' : '';
                });
            };

            socket.on('connect', () => {
                // Join user room + shared feed for presence broadcast.
                socket.emit('join', uid);
                socket.emit('joinFeed');
                // Refresh DB timestamp immediately on connect.
                presencePing();
            });

            socket.on('presence:snapshot', (data) => {
                const ids = (data && data.online_user_ids) ? data.online_user_ids : [];
                if (!Array.isArray(ids)) return;
                ids.forEach((id) => window.fbPresenceApply(id, true));
            });

            socket.on('presence:update', (data) => {
                const id = data && data.user_id;
                const online = !!(data && data.online);
                window.fbPresenceApply(id, online);
            });
        } catch (_e) {
            // silent
        }
